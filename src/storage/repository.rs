use crate::models::{Alert, PriceHistory, Product};
use chrono::Utc;
use sqlx::SqlitePool;
...
impl Repository {
    ...
    pub async fn add_product(
        ...
        let now = Utc::now();
        let res = sqlx::query(
            "INSERT INTO products (title, url, website, target_price, active, created_at) VALUES (?, ?, ?, ?, 1, ?)",
        )
        .bind(title)
        .bind(url)
        .bind(website)
    ...
    pub async fn list_products(&self) -> Result<Vec<Product>, sqlx::Error> {
        sqlx::query_as::<_, Product>(
            "SELECT id, title, url, website, target_price, active, created_at FROM products",
        )
        .fetch_all(&self.pool)
        .await
    }
    ...
    pub async fn get_product_by_id(&self, id: i64) -> Result<Option<Product>, sqlx::Error> {
        sqlx::query_as::<_, Product>(
            "SELECT id, title, url, website, target_price, active, created_at FROM products WHERE id = ?",
        )
        .bind(id)
    ...
    pub async fn get_product_by_url(&self, url: &str) -> Result<Option<Product>, sqlx::Error> {
        sqlx::query_as::<_, Product>(
            "SELECT id, title, url, website, target_price, active, created_at FROM products WHERE url = ?",
        )
        .bind(url)
        .fetch_optional(&self.pool)
        .await
